//Двоичная куча, задача про максимальный элемент в окне
...
template <typename Comparator = std::greater<elem_heap>>
class Heap
{
public:
	Heap(elem_heap* arr, int n) : real_size(n), size(2 * n)
	{
		mas = new elem_heap[size];
	...
	Heap(int* arr, int n) : real_size(n), size(2 * n)
	{
		mas = new elem_heap[size];
		for (int i = 0; i < real_size; ++i)
		{
			this->mas[i].value = arr[i];
			this->mas[i].index = i;
		}
		this->build_heap();
	}
...
private:
	...
	{
		size *= 2;
		elem_heap* next = new elem_heap[size];
	...
	void swap(elem_heap& a, elem_heap& b)
	{
		elem_heap tmp;
		tmp = a;
		a = b;
...
int main() {
	...
	Heap <DefaultComparator<elem_heap>> wind(Mas, n);
	int left = 0;
	int right = window - 1;
...
int* logik(int* Mas, int window, int n)
{
	...
	Heap <DefaultComparator<elem_heap>> wind;
